...
export default function PublicLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <div className="public-site min-h-screen bg-white text-slate-950">
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8"><Brand />
      <nav aria-label="Public navigation" className="hidden items-center gap-8 lg:flex">{links.map(([label, href]) => label === "Fixtures" ? <FixturesNav key={label} /> : label === "Results" ? <PastResultsNav key={label} /> : <ActiveNavLink key={label} label={label} href={href} />)}</nav>
      ...
      <details className="relative lg:hidden"><summary className="grid size-10 cursor-pointer list-none place-items-center border border-slate-200" aria-label="Open menu"><Menu className="size-5" /></summary><nav className="absolute top-12 right-0 flex w-64 flex-col border border-slate-200 bg-white p-3 shadow-xl">{links.map(([label, href]) => <ActiveNavLink key={label} label={label} href={href} mobile />)}<Link href="/admin/dashboard" className="mt-2 bg-amber-400 px-3 py-3 font-black text-xs uppercase">Portal login</Link></nav></details>
    </div></header>
    <main>{children}</main>
