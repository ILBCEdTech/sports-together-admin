...
export function TeachersBySport({ sports }: { sports: SportStaff[] }) {
  ...
  return <Tabs defaultValue={String(sports[0].id)} className="mt-10">
    ...
    {sports.map((sport) => <TabsContent key={sport.id} value={String(sport.id)} className="mt-6 space-y-8">
      {sport.coaches.length > 0 && <section><div className="overflow-hidden border border-slate-300 bg-white"><Table><TableHeader><TableRow><TableHead>{sport.name}</TableHead><TableHead>Match Commissioner</TableHead></TableRow></TableHeader><TableBody>{sport.coaches.map((coach) => <TableRow key={coach.id}><TableCell className="font-medium">{coach.role ?? "Coach"}</TableCell><TableCell>{coach.name}</TableCell></TableRow>)}</TableBody></Table></div></section>}
      {sport.commissioners.length > 0 && <section><h2 className="mb-4 font-serif font-bold text-2xl text-sky-950">Commissioners & Committees</h2><div className="overflow-hidden border border-slate-300 bg-white"><Table><TableHeader><TableRow><TableHead>{sport.name}</TableHead><TableHead>Match Commissioner</TableHead></TableRow></TableHeader><TableBody>{sport.commissioners.map((person) => <TableRow key={person.id}><TableCell className="font-medium">{person.role ?? "Commissioner"}</TableCell><TableCell>{person.name}</TableCell></TableRow>)}</TableBody></Table></div></section>}
    </TabsContent>)}
  </Tabs>;
}
